...
test('deleting every cup leaves the list genuinely empty, not repopulated with defaults', async () => {
  let current = await getCups();
  for (const cup of current) {
    // eslint-disable-next-line no-await-in-loop
    current = await deleteCup(cup.id);
  }

  expect(current).toEqual([]);

  // a fresh read after that should still be empty, not silently reset
  const reread = await getCups();
